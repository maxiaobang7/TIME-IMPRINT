import { ArrowRight, Check, ImagePlus } from "lucide-react";
import { defaultTemplates } from "./data/templates";
import type { WatermarkTemplate } from "./types";

export function StudioHome(props: {
  template: WatermarkTemplate;
  onChoose: (template: WatermarkTemplate) => void;
  onPick: () => void;
  hasPhotos: boolean;
  onContinue: () => void;
}) {
  return (
    <main className="studio-home">
      <div className="home-heading">
        <h1>给宝宝照片，留下成长印记</h1>
        <p>自动读取照片里的拍摄时间与 GPS，<br />为成长瞬间添上日期、地点和那天的小故事。</p>
      </div>
      <section className="home-example" aria-label="水印示例">
        <img src={`/studio/previews/${props.template.id}.webp`} alt={`${props.template.name}水印示例`} fetchPriority="high" />
        <span className="example-label">示例效果</span>
      </section>
      <div className="home-start">
        <button className="primary-action" onClick={props.onPick}>
          <ImagePlus size={20} />
          选择照片，开始制作
        </button>
        {props.hasPhotos && (
          <button className="continue-link" onClick={props.onContinue}>
            继续编辑已选照片
            <ArrowRight size={16} />
          </button>
        )}
        <p>
          自动读取 EXIF · 照片不上传 · 免费使用
        </p>
      </div>
      <section className="home-templates">
        <div className="section-row">
          <h2>选一种喜欢的风格</h2>
        </div>
        <TemplateChoices
          template={props.template}
          onChoose={props.onChoose}
        />
      </section>
      <section className="home-exif" aria-labelledby="home-exif-title">
        <h2 id="home-exif-title">照片里的时间，不用重新填写</h2>
        <p>自动读取原始照片中的 EXIF 拍摄时间与 GPS 坐标，支持本地近似匹配省市。<br />时间、地点和纪念文字，都可以自由修改。</p>
        <p className="exif-caveat">部分截图或转发照片不含 EXIF，可手动补填；离线地点仅供参考。</p>
      </section>
      <footer className="home-footer">
        <span><img className="brand-icon" src="/icons/brand.png?v=girl-v2" alt="" width={32} height={32} />时光印记</span>
        <span>为日常，留一份纪念。</span>
      </footer>
    </main>
  );
}

function TemplateChoices({
  template,
  onChoose,
}: {
  template: WatermarkTemplate;
  onChoose: (item: WatermarkTemplate) => void;
}) {
  return (
    <div className="studio-template-grid home-template-grid">
      {defaultTemplates.map((item) => (
        <button
          key={item.id}
          className={item.id === template.id ? "selected" : ""}
          aria-pressed={item.id === template.id}
          onClick={() => onChoose(item)}
        >
          <div className={`template-sample ${item.category}`}>
            <img src={`/studio/previews/${item.id}-thumb.webp`} alt={`${item.name}水印效果`} loading="lazy" decoding="async" />
          </div>
          {item.id === template.id && <Check className="template-check" size={24} />}
          <span>
            {item.name}
          </span>
        </button>
      ))}
    </div>
  );
}

export function StudioTemplateLibrary(props: {
  template: WatermarkTemplate;
  onChoose: (template: WatermarkTemplate) => void;
}) {
  return (
    <>
      <header className="page-heading">
        <span className="eyebrow">为回忆挑一件外衣</span>
        <h1>照片模板</h1>
      </header>
      <TemplateChoices {...props} />
    </>
  );
}
